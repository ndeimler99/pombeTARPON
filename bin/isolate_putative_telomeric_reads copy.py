...
def get_read_list(fh):
    telo_list = []
    with open(fh, "r") as reads:
        linecount = 0
        for line in reads:
            if linecount == 0:
                linecount += 1
                continue
            line = line.strip()
            telo_list.append(line)
    return telo_list

def get_blast_results(blast_fh, length, identity):
    
    blast_dict = {}
    with open(blast_fh, "r") as blast:
        for line in blast:
            line = line.strip().split()
            # if element is greater than x% of length of element and identity is greater than identitity
            if True:
...
def isolation_putative(args):
    ...
    telomeric = get_read_list(args.repeat_file)
    blast_dict = get_blast_results(args.blast_out)

    aln_file = pysam.AlignmentFile(args.input_file, "rb", check_sq=False)
    putative_reads = pysam.AlignmentFile(args.putative_reads, "w", template=aln_file)
    chimeric_reads = pysam.AlignmentFile(args.chimeric_reads, "w", template=aln_file)
    non_telo_reads = pysam.AlignmentFile(args.non_telo, "w", template=aln_file)
    ...
    for aln in aln_file:
        if aln in telomeric:
            ...
            if g_count > args.repeat_count:
                ...
                non_telo_reads.write(aln)
                continue
            putative_reads.write(aln)
        elif aln in blast_dict:
            print(aln)
